    ...
    if (nCode >= 0) {
        ...
        if ((wParam == WM_NCRBUTTONDOWN) || (wParam == WM_NCRBUTTONUP)) {
            ...
            if (info->wHitTestCode == HTCLIENT) {
				...
				BOOL controlKeyDown = (GetKeyState(VK_CONTROL) & 0x8000) ? TRUE : FALSE;
                BOOL isHitMin = (info->wHitTestCode == HTMINBUTTON) || ((info->wHitTestCode == HTCAPTION) && shiftKeyDown);
				BOOL isHitMax = (info->wHitTestCode == HTMAXBUTTON) || ((info->wHitTestCode == HTCAPTION) && controlKeyDown);
				BOOL isHit = isHitMin || isHitMax;
                ...
                if ((wParam == WM_NCRBUTTONDOWN) && isHit) {
                    ...
                    _hLastHit = info->hwnd;
                    return 1;
                }else if ((wParam == WM_NCRBUTTONUP) && isHitMin) {
                    ...
                    if (info->hwnd == _hLastHit) {
						...
						//PostMessage(FindWindow(NAME, NAME), WM_TOP, 0, (LPARAM)info->hwnd);
                    }
                    _hLastHit = NULL;
                    return 1;
                } else if ((wParam == WM_NCRBUTTONUP) && isHitMax) {
					//DEBUG_PRINTF("%s(%d): up hit\n", __FUNCTION__, __LINE__);
					if (info->hwnd == _hLastHit) {
						//DEBUG_PRINTF("%s(%d): up hit match\n", __FUNCTION__, __LINE__);
						PostMessage(FindWindow(NAME, NAME), WM_TOP, 0, (LPARAM)info->hwnd);
